...
- (no "unknown" state) - vehicle-service starts the FSM in `stand-by`; a persisted `vehicle:state` is re-applied on boot, so no initial or uninitialized string is ever published
...
stateDiagram-v2
    [*] --> stand-by
    %% stand-by is the FSM's initial state. A persisted vehicle:state is
    %% re-applied on boot with SetState, which is not an FSM transition.
    ...
    waiting-hibernation-seatbox --> parked: seatbox button / unlock / kickstand raised (not ready)
    waiting-hibernation-seatbox --> ready-to-drive: kickstand raised (dashboard ready + handlebar unlocked)
