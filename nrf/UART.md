...
- 0x0021: Vehicle state (0=stand-by, 1=parked, 2=ready-to-drive, 3=shutting-down, 4=updating, 5=off)
  - Hop-on mode has no dedicated wire value: bluetooth-service tracks `vehicle:hop-on-active` separately and sends `0` (stand-by) instead of `1` (parked) while it is set, so the nRF52 and BLE clients see a "locked" scooter. Combo-learning entry never sets the flag, so it sends `1` (parked) unchanged.
