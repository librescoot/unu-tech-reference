...
- `redis` → `redis.service` (no map entry; passed through with `.service` appended)
...
- Reads firmware versions from the `system` hash (`mdb-version`, `dbc-version`, `nrf-fw-version`, `environment`)
