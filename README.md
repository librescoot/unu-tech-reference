...
A same-day follow-up to v1.0.0, tagged about nine hours later, so most services
did not move. The theme is hop-on becoming a first-class state.

- Hop-on becomes a published state family: vehicle-service models `hop-on` and
  `hop-on-learning` as leaf states under an at-rest parent with declarative
  event gating, the dashboard parses them from `vehicle:state`, and
  uplink-service maps them to cloud-facing equivalents
- The nRF gains a dedicated `HOP_ON` wire value, so bluetooth-service no longer
  has to present stand-by in software
- BLE power-state changes are inhibited during an nRF firmware flash, full nRF
  state is recovered after a USOCK reconnect, and incompatible firmware is
  upgraded through DFU instead of wedging
- The GPS info grid is shown during cold-start search
- update-service matches real stable filenames when picking a Mender artifact

[Release notes](https://github.com/librescoot/librescoot/releases/tag/v1.0.1)
