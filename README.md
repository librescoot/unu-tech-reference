...
Hotfix for a first-installation problem. Ships service revisions identical to
v1.2.0. The only change is in the Yocto layer: dropbear keeps SHA-1 `ssh-rsa`
enabled and gains legacy `ssh-dss` support, so cross-version ssh continues to
