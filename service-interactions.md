...
arrives. A single watchdog bounds that hold: it expires 15 minutes after the last change to any
`ota` hash field ending in `:dbc`, and then clears the stuck state. There is no
heartbeat-driven short timeout in this release.
...
| `dbc-update` | suspend-only | vehicle-service | a DBC update holds dashboard power |
 It is `suspend-only` rather than `block` so a
