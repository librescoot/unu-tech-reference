...
- Online map tiles (VersaTiles Shortbread, `tiles.versatiles.org`) as alternative
...
| `gps` | GpsStore | 5 s (plus pushed `gps:tpv` snapshots at 1 Hz) |
...
- `settings`: `dashboard.saved-locations.<n>.*` and `dashboard.recent-destinations.<n>.*`, when a saved location or recent destination is deleted

Destination clears do **not** use HDEL: `navigation` `destination`, `latitude`, `longitude`, `address` and `timestamp` are HSET to empty strings, because HDEL does not publish a notification and subscribers would miss the change.
