...
- `status` - Derived internet connectivity status ("connected", "disconnected") - determined by a TCP:53 reachability probe over the modem interface against 8.8.8.8, 1.1.1.1, 9.9.9.9 and 208.67.222.222
- `ip-address` - IPv4 address of the configured modem interface (`wwu1i5` by default); "UNKNOWN" when not connected
...
- `snr` - Average SNR of the satellites used in the fix, dBHz (falls back to visible-satellite average while searching)
- `eps` - Estimated speed error in m/s
- `ept` - Estimated time precision in seconds
- `satellites-used` - Satellites used in the current fix
- `satellites-visible` - Satellites visible to the receiver
...
- `scooter:modem` - Modem power commands (pm-service pushes `disable` before hibernation)
  - `enable` - Mark the modem as wanted; it is brought back up on the next monitor tick
  - `disable` - Close GPS, publish `internet[status]=disconnected`, `internet[modem-state]=off` and `modem[power-state]=off`, then power the modem down over GPIO
...
- **Control:** Via the ModemManager D-Bus API (godbus) and AT commands relayed through it
...
- **SUPL server:** supl.google.com:7276 (plain-TCP SUPL port; only programmed when switching to UE-Based mode, which is disabled in this release)
...
- **Started by:** systemd at boot (WantedBy: multi-user.target; Requires redis.service, After redis/dbus/ModemManager)
- **Restart policy:** `Restart=always` with `KillMode=mixed`; no `RestartSec`, so systemd's 100ms default applies
- **Type:** Simple
...
- **Connectivity test:** TCP connect to port 53 on 8.8.8.8, 1.1.1.1, 9.9.9.9 and 208.67.222.222, each socket bound to the modem interface
...
6. Starts monitoring goroutine with three timers:
   ...
   - GPS update timer (1s)
   - Cell-location timer (5s; only acts when `modem.cell-location` is enabled and there is no GPS fix)
...
2. **Derived internet status** (`status` field):
   - "connected" - Modem connected AND at least one TCP:53 probe target answers
   - "disconnected" - Modem not connected OR every probe target is unreachable

**Connectivity test:**

- **Method:** TCP dial to 8.8.8.8:53, 1.1.1.1:53, 9.9.9.9:53, 208.67.222.222:53 in that order, each socket pinned to the modem interface with `SO_BINDTODEVICE`; first success wins
- **Timeout:** 2 seconds per target (dial timeout)
- **Trigger recovery:** After 3 consecutive failed checks while the modem still reports connected
...
3. **ModemManager Location Sources:**
   ...
   - Enable gps-unmanaged (GPS itself is driven directly over AT)
   - Set GPS refresh rate to 1 second
   - Restart gpsd service

   agps-msb is deliberately not enabled - the SimTech ModemManager plugin does not act on it. The SUPL server (`AT+CGPSURL`) is only programmed when switching to UE-Based mode, which is compiled off in this release, so the receiver runs standalone.

4. **Connect to gpsd:**
   - Subscribe to SKY reports (for DOP values: HDOP, VDOP, PDOP)
   - Subscribe to TPV reports (for position data)
   - Monitor fix mode: 1=none, 2=2D, 3=3D; mode 0 (no value seen) is ignored so partial TPVs don't flicker the fix state
...
1. **No data:** No TPV/SKY report received for 5 seconds (`gps_no_data`)
2. **Fix timeout:** No fix established for 15 minutes since GPS was enabled (`gps_fix_timeout`); disarmed once a fix is achieved
...
- Internet connectivity check fails 3 consecutive times while the modem reports connected
- Data session stalled: modem non-connected but still registered (home/roaming) for 15 minutes
...
- Verify modem health
- Fallback to a ModemManager D-Bus reset if GPIO fails
...
Modem status (changed fields are batched into one line per hash, `key=value` style):

- "internet status=connected modem-state=connected ip=10.1.2.3 tech=4G signal=72"
- "modem power=on sim=present reg=home connectivity=online error=ok"

GPS events:

- "Waiting for valid GPS fix..."
- "GPS fix established"
- "gps state=fix-established fix=3d eph=8.5m hdop=0.9 vdop=1.4 pdop=1.7 snr=34.2dBHz sats=9/14" (logged every 90 seconds)
- "gps ttff=42.1s mode=standalone snr=34.2dBHz sats=9/14" (once per acquisition)
...
- "modem-service v0.12.0-1-gc193d67"
...
- **ModemManager** - For modem control and status (D-Bus API on the system bus)
- **chrony** - `chronyc settime` is used to seed the system clock from GPS
...
- **GPIO access** - gpiochip3 line 14 (GPIO4.14, pin 110) via the gpiocdev character-device ABI (`/dev/gpiochip3`)
...
- **github.com/godbus/dbus/v5** v5.2.2 - ModemManager D-Bus client
- **github.com/librescoot/redis-ipc** v0.14.0 - Redis IPC layer (pulls in github.com/redis/go-redis/v9 v9.18.0 indirectly)
- **github.com/stratoberry/go-gpsd** v1.3.0 - GPSD client
- **github.com/warthog618/go-gpiocdev** v0.9.1 - GPIO character-device access for modem power
