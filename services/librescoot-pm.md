...
**Note:** The hibernation timer default is 72 hours (3 days). The shipped `librescoot-pm.service` starts the binary with `-default-state run`, and a valid `pm.default-state` value in the `settings` hash overrides the flag at startup and at runtime.
...
- `pm.hibernation-timer` - Hibernation timer duration in seconds (0 disables the timer; negative values are ignored)
- `pm.default-state` - Overrides the `-default-state` flag; ignored unless it is one of run/suspend/hibernate/hibernate-manual/hibernate-timer/reboot
...
- Reads `settings pm.default-state` to override the `-default-state` flag
