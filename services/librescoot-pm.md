...
- `pm.suspend-when-online` - Bool, default `false`. Only matters when no main battery is present: a pack present (or active) in either slot always blocks suspend (`Cannot enter suspend state: a main battery is present or active`), independent of this setting. With no main battery, the default (`false`) keeps an online scooter awake so cloud commands can still reach it, blocking suspend with `Suspend blocked: no main battery but online and pm.suspend-when-online disabled`. When `true`, a scooter with no main battery may suspend even while online. The guard only applies to the `suspend` target (not hibernate/reboot). Present and active are read live from Redis at the decision point because pub/sub state is lost across a suspend freeze.
