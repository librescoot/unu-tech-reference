...
**Fields read on startup:** `wake-cause` (deleted after consumption). Intended as a durable backstop for the wake-from-hibernation indicator across the startup-ordering race with motion-service's pub/sub. In this release the read is dead code: motion-service does not write the field yet, so `ConsumeWakeCause` always returns false and the live `motion:interrupt` event is the only path.
...
- **armed**: Armed and monitoring motion events plus unauthorized seatbox openings
