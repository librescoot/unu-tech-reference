...
--evdev-keycode=0x2b           Keycode from the gpio-keys device for BMX055 INT1
--poller-interval-ms=1000      Interval in ms between I2C status polls (watchdog
                               when evdev is active, primary source otherwise)
...
- `status` - Current alarm status:
  ...
  - `delay-armed` - 5-second arming delay in progress
...
**Fields written once at startup and never updated afterwards:**
...
Per-state sensor and interrupt-pin configuration is applied directly over I2C and
is not mirrored into this hash, so these values do not track the alarm state.
...
- `scooter:power` - `hibernate-manual`, sent to pm-service to re-enter hibernation
  after the post-wake quiet period elapses
...
- `power-manager` - Monitors "state"; a hibernating/hibernating-imminent value selects
  the stricter armed BMX profile
...
- Vehicle enters `parked`, `ready-to-drive` or `waiting-seatbox` (any other vehicle
  state leaves the alarm armed)
- User disables alarm (`LPUSH scooter:alarm disable`)
- Runtime disarm (`LPUSH scooter:alarm disarm`)

`LPUSH scooter:alarm stop` only silences the horn and hazards; it does not change
the FSM state.
...
- **Gyroscope (0x68):** Probed at startup and soft-reset alongside the accelerometer; not used for motion detection or interrupt validation
- **I2C Bus:** Default `/dev/i2c-3` (configurable via `--i2c-bus`)
- **Interrupt path:** the gpio-keys evdev edge on BMX055 INT1 is the primary, zero-latency source (`--evdev-device`, `--evdev-keycode`); an I2C status poll at `--poller-interval-ms` (default 1000ms) runs alongside it as a watchdog and becomes the only source when the evdev device cannot be opened

### BMX055 Configuration

The service automatically configures BMX sensitivity based on alarm state:

| State | Wake Lock | Sensor profile (engine / bandwidth / threshold / duration) | INT Pin |
|-------|-----------|-----------------------------------------------------------|---------|
| delay_armed | Yes | slow-motion / 7.81 Hz / 0x14 / 0x02 | INT2 |
| armed | No | any-motion / 31.25 Hz / 0x06 / 0x03 (threshold 0x08 when hibernation is imminent) | BOTH (INT1 for polling, INT2 for nRF wake) |
| trigger_level_1_wait | Yes | unchanged (soft reset only) | unchanged |
| trigger_level_1 | Yes | slow-motion / 15.63 Hz / 0x08 / 0x03 | BOTH (INT1 for polling, INT2 for nRF wake) |
| trigger_level_2 | Yes | unchanged (soft reset only) | unchanged |
| waiting_movement | Yes | slow-motion / 7.81 Hz / 0x06 / 0x03, applied 47s into the cycle | NONE |
| seatbox_access | Yes | slow-motion / 7.81 Hz / 0x14 / 0x02, interrupts disabled | NONE |

**Note:** BMX055 accelerometer bandwidth varies by state: 7.81 Hz (PMU_BW 0x08) for the idle and L2-waiting profiles, 15.63 Hz (0x09) for the Level 1 check, and 31.25 Hz (0x0A) for both armed profiles.
...
5. Starts the I2C interrupt poller (default 1000ms) and, if `--evdev-device` opens, the evdev interrupt watcher
...
5. Cooldown period (default 5s, `settings alarm.l1-cooldown` / `--l1-cooldown`)
...
6. Maximum 6 `trigger_level_2`/`waiting_movement` cycles (roughly 10 minutes)
7. State: `trigger_level_2` → `disarmed`, then a 5-minute post-alarm cooldown before re-arming
...
`LPUSH scooter:alarm stop` only stops the horn and hazard output; the FSM stays
where it is.
...
- **trigger_level_1_wait**: Wake lock acquired for the Level 1 cooldown, held through `trigger_level_1`
- **trigger_level_2**: Wake lock acquired for the full alarm, held through `waiting_movement`
- **seatbox_access**: Wake lock acquired for the duration of an authorized seatbox opening
...
```bash
# Force arming from disarmed (enters delay_armed, so the 5-second delay still applies; doesn't change alarm.enabled)
