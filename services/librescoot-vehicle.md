...
- (no `horn:button` field: horn button presses are written to the `buttons` hash as `horn:on` / `horn:off`, not to the `vehicle` hash)
- (no `seatbox:button` field: seatbox button presses are written to the `buttons` hash as `seatbox:on` / `seatbox:off`, not to the `vehicle` hash)
...
- `scooter:hop-on` - Hop-on mode commands:
  - `engage` - Enter the locked hop-on state (lock screen, LED cue, opportunistic steering lock; `vehicle:state` stays `parked`, `vehicle:hop-on-active` is published `true`)
  - `engage-silent` - Enter combo-learning quietly, reusing the same FSM state (no LED cue, no steering lock, no lock screen, `hop-on-active` is never published)
  - `release` - Exit hop-on back to `parked`
- `scooter:led:cue` - LED cue playback commands (integer cue index)
- `scooter:led:fade` - LED fade playback commands ("channel:fadeIndex")
- `scooter:update` - Update commands ("start", "complete", "start-dbc", "complete-dbc"). Anything else, including "cycle-dashboard-power", is rejected by the list handler before it reaches the state machine.
...
- `hop-on` - Hop-on/hop-off mode (rider briefly off, motor disabled, optional steering lock, lock screen rendered by dashboard). FSM-internal id; `vehicle:state` keeps reading `parked` throughout, with `vehicle:hop-on-active` published separately. Also entered silently for combo-learning (no LED cue, no steering-lock attempt, no lock screen, no `hop-on-active` publish); input suppression for both variants (no horn / blinker LED / brake LED / seatbox open / hibernation hold) is handled by hand in `handleInputChange`, not via declarative per-state event blocking.
...
- Kickstand up
- Handlebar unlocked (has been unlocked at least once this power cycle)
