...
- `type` - `scooter`
- `uid` - UID of the card that authenticated. Each successful auth also sets a 10-second TTL on the whole `keycard` key, so every field in the hash expires together, including a `command-result` written while the TTL is still running.
